...
/**
 * https://github.com/FeiMo1988/Scanner
 */
public class IpScanner extends Thread {

    private static final String TAG = IpScanner.class.getSimpleName();
    ...
    public void run() {
        try {
            ...
            ipHost = ip.toString();
            if (ip.isFull() && mask.isFull()) {
                Log.d(TAG, "host ip:" + ipHost);
                Log.d(TAG, "host mask:" + mask.toString());
                int v = mask.toInt();
                int vip = ip.toInt();
                int begin = (vip & v);
                v = ~v;
            ...
            if (mScanCallback != null) {
                mScanCallback.onFoundError(LO_IP, this.mPort, e);
            }
        }
    }
        ...
        public void run() {
            ...
            while (true) {
                try {
                        ...
                        if (sKey.isConnectable()) {
                            ...
                            try {
                                ...
                                if (channel.finishConnect()) {
                                    mResult.add(ip);
                                    mScanCallback.onFoundOne(ipHost, mPort, ip);
                                }
                                channel.close();
                            } catch (Exception e) {
                            }
                    ...
                    Log.d(TAG,Thread.currentThread() + "size = " + this.channels.size());
            ...
            Log.d(TAG,Thread.currentThread() + " count = " + v);
            if (v == 0 && !mCallbackCalled.getAndSet(true)) {
                Log.d(TAG,"scan use time = " + (SystemClock.uptimeMillis() - startTimeMillis) / 1000 + "s");
                mScanCallback.onFoundDone(ipHost, mPort, mResult);
            }
