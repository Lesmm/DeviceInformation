...
public class IProcessUtil {
	...
	public static String execCommandsWithSu(String command, String... subCommands) {
		...
		if (process != null) {
			process.destroy();
			Log.d("DeviceInfo", "Execute su command done: " + command + " for pid: " + pid);
		}
		return result;
	...
	public static Process processCommandsWithSu(String command, String... subCommands) {
		Log.d("DeviceInfo", "Process su command: " + command);
		int pid = -1;
		...
		try {
			...
			Log.d("DeviceInfo", "Process su command: " + command + " in pid: " + pid);
			...
			for (int i = 0; i < subCommands.length; i++) {
				...
				Log.d("DeviceInfo", "Process su subcommand: " + subCommand);
			...
			if (process != null) {
				process.destroy();
				process = null;
				Log.d("DeviceInfo", "Process su command exception: " + command + " for pid: " + pid);
			}
	...
	public static String execCommands(String command, String... subCommands) {
		...
		if (process != null) {
			process.destroy();
			Log.d("DeviceInfo", "Execute command done: " + command + " for pid: " + pid);
		}
		return result;
	...
	public static Process processCommands(String command, String... subCommands) {
		Log.d("DeviceInfo", "Process command: " + command);
		int pid = -1;
		java.lang.Process process = null;
		try {
			process = Runtime.getRuntime().exec(command);
			pid = getPidFromJavaProcess(process);
			Log.d("DeviceInfo", "Process command: " + command + " in pid: " + pid);
			...
			for (int i = 0; i < subCommands.length; i++) {
				String subCommand = subCommands[i];
				Log.d("DeviceInfo", "Process subcommand: " + subCommand);
				dataOutputStream.writeBytes(subCommand + "\n");
			}
			...
			if (process != null) {
				process.destroy();
				process = null;
				Log.d("DeviceInfo", "Process command exception: " + command + " for pid: " + pid);
			}
	...
	public static String executeCommandWithArgs(String... command) {
		String result = "";
		Log.d("DeviceInfo", "Execute Command With Args: " + command);
		int pid = -1;
		java.lang.Process process = null;
		try {
			process = new ProcessBuilder().command(command).start(); // ProcessManager.getInstance().exec(...);
			pid = getPidFromJavaProcess(process);
			Log.d("DeviceInfo", "Execute Command With Args: " + " for pid: " + pid);
			...
			try {
				if (process != null) {
					process.destroy();
					Log.d("DeviceInfo", "Execute Command With Args: " + command + " for pid: " + pid);
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public static int getPidByUid(int uid) {
		...
		Log.d("DeviceInfo", "-----> GetPidByUid(" + uidValue + ") -> " + pid);
		return pid;
	}
	...
	public static boolean killProcessByPid(int pid) {
		Log.d("DeviceInfo", "-----> Killing process pid: " + pid);
		...
		while (checkIfExistedCount > 0) {
			...
			Log.d("DeviceInfo", "-----> Checking process pid: " + pid + " is existed: " + isPidExisted + " -> " + checkIfExistedCount);
		...
		Log.d("DeviceInfo", "-----> KillProcessByPid(" + pid + ") -> " + (isPidExisted ? "failed" : "success"));
		return isPidExisted;
	}
