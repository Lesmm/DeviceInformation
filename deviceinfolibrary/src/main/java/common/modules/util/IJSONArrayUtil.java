...
public class IJSONArrayUtil {
    ...
    public static JSONArray createJSONArrayFromFile(String jsonFile) {
        String string = IFileUtil.readFileToText(jsonFile);
        JSONArray json = null;
        if (string != null) {
    ...
    public static ArrayList jsonToList(JSONArray json) {
        ...
        for (int i = 0; json != null && i < json.length(); i++) {
            ...
            if (value instanceof JSONObject) {
                value = IJSONObjectUtil.jsonToMap((JSONObject) value);
    ...
    public static ArrayList<HashMap<String, Object>> toListMap(JSONArray jsonArray) {
        ...
        for (int i = 0; i < jsonArray.length(); i++) {
            ...
            if (value instanceof JSONObject) {
                ...
                Map<String, Object> map = IJSONObjectUtil.jsonToMap(element);
                list.add(map);
            }
        }
        return list;
    ...
    public static void replaceJsonElementsValues(JSONArray sourceJson, JSONArray replaceJson) {
        ...
        for (int i = 0; i < min; i++) {
            ...
            if (val instanceof JSONObject && replaceVal instanceof JSONObject) {
                IJSONObjectUtil.replaceJsonElementsValues((JSONObject) val, (JSONObject) replaceVal);
            } else if (val instanceof JSONArray && replaceVal instanceof JSONArray) {
                IJSONArrayUtil.replaceJsonElementsValues((JSONArray) val, (JSONArray) replaceVal);
            } else {
                IJSONArrayUtil.put(sourceJson, i, replaceVal);
            }
        }

        for (int i = min; i < replaceJson.length(); i++) {
            if (sourceJson.length() <= i) {
                continue;
            }
